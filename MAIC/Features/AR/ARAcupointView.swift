...
struct ARAcupointView: View {
    ...
    private func markerOverlay(in size: CGSize) -> some View {
        ForEach(Array(markers.enumerated()), id: \.element.id) { i, point in
            let cur = isGuided && point.id == current?.id
            let pos = position(for: point, index: i, total: markers.count, in: size)
            VStack(spacing: 6) {
                BreathingDot(size: cur ? 34 : 22, labelInitial: nil)
                    .scaleEffect(cur ? 1.0 : 0.9)
                Text(point.nameZh)
                    .font(.caption2.weight(.bold))
                    .foregroundStyle(.white)
                    .padding(.horizontal, 8).padding(.vertical, 3)
                    .background(.ultraThinMaterial, in: Capsule())
                    .opacity(isGuided && !cur ? 0.45 : 1)
            }
            .position(pos)
